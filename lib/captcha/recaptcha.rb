module Captcha
  ...
  private
    def verify_settings(settings, key)
      ...
      (raise Captcha::CaptchaError, "Missing private key" if @global_settings[:private_key].nil?) if key == :private_key
      (raise Captcha::CaptchaError, "Missing public key" if @global_settings[:public_key].nil?) if key == :public_key
    end
